...
std::u16string Version::ToString(int32_t fieldCount) const
{
    ...
    case 1:
        c8StrLen = TG_SPRINTF(c8Buffer, "%d", major_);
        break;

    case 2:
        c8StrLen = TG_SPRINTF(c8Buffer, "%d.%d", major_, minor_);
        break;
    ...
    default:
        ...
        if (fieldCount == 3)
        {
            c8StrLen = TG_SPRINTF(c8Buffer, "%d.%d.%d", major_, minor_, build_);
            break;
        }
        ...
        if (fieldCount == 4)
        {
            c8StrLen = TG_SPRINTF(c8Buffer, "%d.%d.%d.%d", major_, minor_, build_, revision_);
            break;
        }
