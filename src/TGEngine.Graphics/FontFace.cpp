...
std::shared_ptr<FontFace> FontFace::Create(const std::shared_ptr<std::remove_pointer_t<FT_Library>>& library, const std::span<const std::byte>& fileData, int32_t fontSize)
{
    ...
    return std::make_shared<FontFace>(decltype(_fontFace)(face, [](FT_Face face)
    {
        FT_Done_Face(face);
    }));
}
...
{
    const auto it = _glyphData.find(c);
    if (it != _glyphData.cend())
        return &it->second;

    if (FT_Load_Char(_fontFace.get(), c, FT_LOAD_RENDER))
        return nullptr;

    constexpr std::byte whiteColorPixel[4] {std::byte(255), std::byte(255) ,std::byte(255), std::byte(255)};

    const I32Extent2D bitmapExtent(static_cast<int32_t>(_fontFace->glyph->bitmap.width), static_cast<int32_t>(_fontFace->glyph->bitmap.rows));
    ...
    for (int64_t i = 0, j = 0; i < static_cast<int64_t>(bitmapExtent.width) * bitmapExtent.height * 4; i += 4, ++j)
    {
        ...
        bitmap[i + 3] = std::byte(_fontFace->glyph->bitmap.buffer[j]);
    }

    const auto bearing = IntVector2(static_cast<int32_t>(_fontFace->glyph->bitmap_left), static_cast<int32_t>(_fontFace->glyph->bitmap_top));
    const auto advance = IntVector2(static_cast<int32_t>(_fontFace->glyph->advance.x >> 6), static_cast<int32_t>(_fontFace->glyph->advance.y >> 6));
    return &_glyphData.insert(it, {c, GlyphData{c, GlyphMetrics{bitmapExtent, bearing, advance}, std::move(bitmap)}})->second;
}

IntVector2 FontFace::GetKerning(char32_t lhs, char32_t rhs) const
{
    const auto lhsIndex = FT_Get_Char_Index(_fontFace.get(), lhs);
    const auto rhsIndex = FT_Get_Char_Index(_fontFace.get(), rhs);

    FT_Vector kerning;
    if (FT_Get_Kerning(_fontFace.get(), lhsIndex, rhsIndex, FT_KERNING_DEFAULT, &kerning))
        return {};
