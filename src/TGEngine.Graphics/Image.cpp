...
void FlipImageX(std::byte* imageData, int32_t width, int32_t height)
{
    ...
    for (int32_t y = 0; y < height; ++y)
    {
        ...
        for (; frontIt < backIt; ++frontIt, --backIt)
        {
            const auto r1 = _m_shuffle_epi32(_m_loadu_si128(frontIt), _M_SHUFFLE(0, 1, 2, 3));
            const auto r2 = _m_shuffle_epi32(_m_loadu_si128(backIt), _M_SHUFFLE(0, 1, 2, 3));
            _m_storeu_si128(frontIt, r2);
            _m_storeu_si128(backIt, r1);
        }
    }
#else
...
std::optional<Image> Image::Create(const std::span<const std::byte>& bytes)
{
    ...
    auto imageData = std::unique_ptr<std::byte[]>(reinterpret_cast<std::byte*>(stbi_load_fro_memory(reinterpret_cast<const stbi_uc*>(&bytes[0]), static_cast<int>(bytes.size()), &width, &height, nullptr, STBI_rgb_alpha)));
    if (imageData == nullptr)
    {
        return {};
    }
...
bool Image::Save(const char8_t* filePath, ImageFormat format) const
{
    ...
    case ImageFormat::Png:
        return stbi_write_png(reinterpret_cast<const char*>(filePath), _size.width, _size.height, 4, _imageData.get(), _size.width * 4) != 0;

    case ImageFormat::Jpeg:
        return stbi_write_jpg(reinterpret_cast<const char*>(filePath), _size.width, _size.height, 4, _imageData.get(), 100) != 0;

    case ImageFormat::Bmp:
        return stbi_write_bmp(reinterpret_cast<const char*>(filePath), _size.width, _size.height, 4, _imageData.get()) != 0;
...
void Image::RotateFlip(RotateFlipType rotateFlipType)
{
    ...
    case RotateFlipType::Rotate90FlipNone:
        RotateImageRight90Degrees<BytesPerPixel>(_imageData.get(), _size.width, _size.height);
        std::swap(_size.width, _size.height);
        break;

    case RotateFlipType::Rotate180FlipNone:
        FlipImageXY<BytesPerPixel>(_imageData.get(), _size.width, _size.height);
        break;

    case RotateFlipType::Rotate270FlipNone:
        RotateImageLeft90Degrees<BytesPerPixel>(_imageData.get(), _size.width, _size.height);
        std::swap(_size.width, _size.height);
        break;

    case RotateFlipType::RotateNoneFlipX:
        FlipImageX<BytesPerPixel>(_imageData.get(), _size.width, _size.height);
        break;
    ...
    case RotateFlipType::RotateNoneFlipY:
        FlipImageY(_imageData.get(), _size.width, _size.height, BytesPerPixel);
        break;
