...
public:
    ...
    int32_t IndexOf(ValueType c, int32_t startIndex = 0) const;
    template <typename _Predicate>
    int32_t IndexOfAny(const _Predicate& predicate, int32_t startIndex = 0) const;
    ...
    int32_t LastIndexOf(ValueType c) const;
    int32_t LastIndexOf(ValueType c, int32_t startIndex) const;
    template <typename _Predicate>
    int32_t LastIndexOfAny(const _Predicate& predicate) const;
    template <typename _Predicate>
    int32_t LastIndexOfAny(const _Predicate& predicate, int32_t startIndex) const;
...
constexpr BasicStringHash<_String>::BasicStringHash(const _String2& str) noexcept :
    _str(str),
    _hashCode(std::hash<BasicStringHash>{}(this->Data()))
{
}
...
{
    return _hashCode == rhs.GetHashCode() && _str == rhs._str;
}

template <typename _String>
constexpr bool BasicStringHash<_String>::operator==(const ValueType* rhs) const noexcept
{
    return _hashCode == std::hash<BasicStringHash>{}(rhs) && BasicStringTraits<ValueType>::Compare(this->Data(), this->Length(), rhs, BasicStringTraits<ValueType>::Length(rhs)) == 0;
}
...
constexpr bool BasicStringHash<_String>::operator<(const BasicStringHash<_String2>& rhs) const noexcept
{
    return _hashCode < rhs.GetHashCode();
}
...
constexpr int32_t BasicStringHash<_String>::Length() const noexcept
{
    if constexpr (detail::IsBasicString<StringType> || detail::IsBasicStringView<StringType>)
    {
        return static_cast<int32_t>(_str.length());
    }
    else
    {
        return static_cast<int32_t>(_str.Length());
